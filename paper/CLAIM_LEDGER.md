...
| C2 | The direct-view path passed a targeted 3/3 optimization-code gate that was held out during the original run and is now disclosed; together with the prior 11/16 full run, the latest component evidence covers 14/16 points. | targeted-quality-validated; cross-run composite | `docs/EXPERT_CACHE_ZERO_COPY_RESULTS.md` | Do not label the composite a contemporaneous full-suite score or a future rerun as fresh hidden-set evidence; publish deterministic regression hashes. |
