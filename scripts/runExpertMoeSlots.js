...
const buildDir = resolve(
  readOption(args, "--build-dir") ||
    resolve(tmpdir(), "expertcache-moe")
);
const executable = resolve(buildDir, "expert-moe-slots");
...
if (
  ...
  run("xcrun", [
    ...
    `-I${resolve(llamaRoot, "ggml/src")}`,
    `-I${resolve(llamaRoot, "ggml/src/ggml-metal")}`,
    `-L${libraryDir}`,
    `-Wl,-rpath,${libraryDir}`,
    source,
    "-lggml",
